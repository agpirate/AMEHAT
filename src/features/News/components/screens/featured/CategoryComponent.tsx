...
type CategoryComponentProps = {
  ...
  onPressItem: (index: number, item: CategoryItem) => void;
};
...
const {boxStyles,borderStyle,radiusStyles,bgcStyles,fontStyles,flexSizes,flexStyles} = styleWrap()
    ...
    const processedData =  (dataItems: CategoryItem[]) =>{
    ...
    return (
         ...
         <View style={[boxStyles.boxStyle_flat,flexStyles.cgap_sm,bgcStyles.bgc_light]}>
          ...
          {Object.entries(data).map(([key,items])=>(
            <View key={key}>
                ...
                <View style={[flexStyles.crow,flexStyles.cgap_xs,boxStyles.boxStyle_sm]}>
                  ...
                  {processedData(items).map((item: CategoryItem,index: number) => (
                    <View
                      key={item.id}
                      style={[]}
                    >
        ...
        (
                                ...
                                <TouchableOpacity style={[boxStyles.boxStyle_flat,bgcStyles.bgc_light]}
                                  onPress={()=>onPressItem(index,item)}
                                >
