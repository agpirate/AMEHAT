...
import { apiService } from '../../../services/apiOperations';
 ...
 const OpsCrundEndDataItemSignUpSlice: StateCreator<AppStore,[],[],OpsCrundEndDataItemSignUpTypes> = (set, get) => ({
  ...
  createCrudEndSignUpYield: async () => {
  ...
  updateCrudEndSignUpYield: async () => {
    const { YieldItem,operation, validateItem } = get();
  ...
  deleteCrudEndSignUpYield: async () => {
...
export default OpsCrundEndDataItemSignUpSlice