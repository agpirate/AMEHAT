...
import OpsCrundEndDataItemSignInTypes from '../../../types/opsCrudEnd/[Yield]Item/OpsCrudEndSignInYieldItemTypes';
import {  AppStore } from '../../../types';

import { apiService } from '../../../services/apiService';

 const OpsCrundEndDataItemSignInSlice: StateCreator<AppStore,[],[],OpsCrundEndDataItemSignInTypes> = (set, get) => ({
  // modelYieldItemGroups,
  
  createCrudEndSignInYield: async () => {
    ...
    if (!('item' in YieldItem)) throw new Error('No item found in YieldItem state');
     ...
     get().setOperationStatus('loading');
      const newItem = await apiService.createItem(get().YieldItem.item);
  ...
  updateCrudEndSignInYield: async () => {
    const { YieldItem,operation, validateItem } = get();
    ...
    if (!('item' in YieldItem)) throw new Error('No item found in YieldItem state');
      ...
      get().setOperationStatus('loading');
      const updatedItem = await apiService.updateItem(YieldItem.itemMeta.id,get().YieldItem.item);
  ...
  deleteCrudEndSignInYield: async () => {
    ...
    fetchCrudEndSignInYield: async () => {
      ...
      //-----------
      get().setapiQFilter(get().YieldItem.item as Record<string,any>)//{'category':{'$eq':'home'}, 'locale':{'$eq':'en'}}
      //-----------
      // // Fetch latest news    
      const items = await apiService.fetchItems(get().apiQueringFilters);
      let item = items.data  ? items.data[0] : {}

      get().setYieldItem({...item,'authenticated':true})
