...
//utils
type HomeScreenProps = StackScreenProps<RootStackParamList,typeof ROUTES.MAIN_TAB.TV.LIST.LATEST>;
...
const {boxStyles,borderStyle,radiusStyles,bgcStyles,fontStyles,flexSizes,flexStyles} = styleWrap()
  ...
  const onPressCategoryFeatureItems = (
    ...
    const navigateTo =(path :string,params ={cat:''}) =>{
      ...
      if(path == ROUTES.MAIN_TAB.TV.DETAIL) navigation.push(ROUTES.MAIN_TAB.TV.DETAIL, {id:'',source:'',highlight:true,title})
      else if ( path == ROUTES.MAIN_TAB.TV.LIST.LATEST) onRetry()
      else   navigation.push(ROUTES.MAIN_TAB.TV.LIST.GROUP_CATEGORY,{categoryId:params.cat,categoryName:params.cat})
      return;
    }
  ...
  const onRetry = React.useCallback(() => {
    //put new filtering
    startFetchOperation({key: 'category', value: 'tigrai tv'}, null);
  ...
  {/* Menus */}
  {(pageSetting['scrollYEvent'] < 100) ?  <CategoryOptions navigateTo={navigateTo}   /> :  null}
