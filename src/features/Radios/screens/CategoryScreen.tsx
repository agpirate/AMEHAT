...
type HomeScreenProps = StackScreenProps<RootStackParamList,typeof ROUTES.MAIN_TAB.RADIO.LIST.GROUP_CATEGORY>;
...
const {boxStyles,borderStyle,radiusStyles,bgcStyles,fontStyles,flexSizes,flexStyles} = styleWrap()
  ...
  const navigateTo =(path :string,params ={cat:''}) =>{
    if(path == ROUTES.MAIN_TAB.RADIO.DETAIL) navigation.push(ROUTES.MAIN_TAB.RADIO.DETAIL, {id:'',source:'',highlight:true})
    else if ( path == ROUTES.MAIN_TAB.RADIO.LIST.LATEST) navigation.push(ROUTES.MAIN_TAB.RADIO.LIST.LATEST,{refresh:true})
    else   onRetry()
    return;
  }
  //for
