...
> = (set, get) => ({
  ...
  //-----------------
  setapiQPage: (page) => set((state) => ({ apiQueryParams: { ...state.apiQueryParams,pagination:{...state.apiQueryParams.pagination,page} } })),
  setapiQItemsPerPage: (count) =>
    set((state) => ({ apiQueryParams: { ...state.apiQueryParams,pagination:{...state.apiQueryParams.pagination,pageSize:count,page:1}} })),
  setapiQPagination: (obj={}) =>
   {
    get().apiQueryParams.pagination = {...get().apiQueryParams.pagination,pageSize:obj.pageSize ?? 2,page:obj.page ?? 1}
   },
