...
import type { DataItem, ApiQueryParams, ApiResponse } from '../types/modelTypes';
...
export const apiService = {
  ...
  fetchItemsHome: async (params?: ApiQueryParams): Promise<ApiResponse<DataItem[]>> => {
    try {
  ...
  fetchItems: async (params?: ApiQueryParams): Promise<ApiResponse<DataItem[]>> => {
    try {
      const response = await request<{
        data: DataItem[];
        pagination: { total: number; pages: number };
